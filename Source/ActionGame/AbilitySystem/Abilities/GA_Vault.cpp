...
bool UGA_Vault::CommitCheck(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, OUT FGameplayTagContainer* OptionalRelevantTags /*= nullptr*/)
{
	...
	int32 JumpToLocationIndx = INDEX_NONE;
	...
	for (;i < HorizontalTraceCount; ++i)
	{
		...
		{
			if (JumpToLocationIndx == INDEX_NONE && (i < HorizontalTraceCount - 1))
			{
				JumpToLocationIndx = i;
				JumpToLocation = TraceHit.Location;
			}
			else if(JumpToLocationIndx == (i -1))
			{
				MaxJumpDistance = FVector::Dist2D(TraceHit.Location, TraceStart);
				break;
			}
		}
		else if (JumpToLocationIndx != INDEX_NONE)
		{
			break;
		}
	...
	if (JumpToLocationIndx == INDEX_NONE)
	{
		return false;
	}
